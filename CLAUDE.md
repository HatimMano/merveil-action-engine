...
⭐⭐ **Porte en `on` + 3 critères + Livrer — chantier D, 2026-09-07 (ADR, décision Hatim).** `ISEO_HOLD_MODE=on` par défaut dans `deploy.sh` (observe = `ISEO_HOLD_MODE=observe ./deploy.sh`). `_evaluate_hold` a **6 critères** : les 3 historiques (direct ≤ 72 h · direct solde dû · paiement refusé sans encaissement) + **combo fraude** (`int_reservations__risk.is_fraud_combo`), **blacklist confirmée** (`int_reservations__blacklist_match`, exact/fort + actionnable), **réservé le jour J** (tous canaux, `DATE(created_at, Paris) = checkin_date`) — lus dans `_STAYS_CTE` (`risk_flags`, `blacklist_flags`), miroir `dash_ops_hold_simulation`. Une rétention = code créé, **pas poussé** ; la RC lit 6.1 (chip 🔒 Hold, motif sous la cellule) et clique **[Livrer le code]** (`POST /api/iseo/release-hold` côté dashboards : push Duve immédiat si le service porte `DUVE_CONNECT_TOKEN`, sinon `released_at` et `_resa_duve_retry` pousse au run suivant ≤ 10 min) ou **[Révoquer]**. ⚠ Le flip est sûr parce que E garantit que le code existe et se lit en 6.1.
