...
- **Critères** (`_evaluate_hold`) : **last-minute ≤ `ISEO_HOLD_LEAD_HOURS` (24h par défaut), tous canaux** — c'est le critère qui porte la valeur, ~24 résas/mois, 3 des 4 fraudes d'août étaient des résas du jour même · **rien d'encaissé sur le canal DIRECT seul**.
...
- ⚠ **Volontairement PAS conditionné à « pièce d'identité scannée »** : scanner une pièce coûte 30 s à un fraudeur (n'importe quel document passe l'OCR) → en faire une condition de libération rendrait la porte contournable par une action que l'attaquant contrôle. La pièce sert au **triage humain** au moment de libérer (et à l'alerte temps réel du webhook-gateway, où le coût d'un raté est faible).
