...
**Label Luckey — guest tag partagé (2026-06-29)** : `_provision`/`_resync` forcent désormais `ISEO_DEFAULT_GUEST_TAG_ID` (132094) au lieu du guest tag dominant de l'appart. Cause : chaque guest tag Sofia porte UN user ; le tag dominant d'un appart appartient à un vrai guest passé → tous nos PIN héritaient de son nom dans l'UI Luckey (ex: PIN de Muller/Henderson affichés « Bianca Aranha » sur SEB23-3G). Le credentialRule scope l'accès par le lockTag → le guest tag n'a aucun effet de bord. ⚠️ **Limite connue** : l'user derrière 132094 est `sylvain@archides.fr` (ROLE_MANAGER réel, **non renommable**) → le label affiché est « INTEGRATION 19 sylvain et hatim », uniforme mais pas idéal. **Backlog B2** : créer un user+tag dédié « Merveil Guest » pour un label propre (nécessite de confirmer l'endpoint Sofia de création user/tag — même blocage que le util `iseo_sync_lock_tags.py`). Le vrai nom guest reste exposé sur le dashboard (dérivé Duve).
