...
- **Critères** (`_evaluate_hold`, recalibrés le 2026-08-15) — **les deux sur le canal DIRECT** : **réservé ≤ `ISEO_HOLD_LEAD_HOURS` (72 h par défaut) avant l'arrivée** (~8/mois, le critère qui porte la valeur : les 4 fraudes d'août sont toutes en direct, 3 réservées le jour même, Bossongo à J-2 — que le seuil 24 h ratait) · **rien d'encaissé**. Mesuré en fenêtre de provisioning le 15/08 : **5 rétentions sur 198 stays** (2,5 %), whitelist non comptée.
