...
- **Étendre à une autre règle** : ajouter l'entrée dans `RULES` (+ `space`) ; bloc vivant = champ `live="<clé>"` + collecteur dans `LIVE_COLLECTORS` (retourne `{generated_at, last_run?, items[]}`).
