...
- **Contrôle de couverture** : `coverage_gap()` logue en fin de run les triggers `enabled` qu'aucune règle ne documente. **9 au 12/08** : `abandoned_cart`, `low_occupation`, `low_review_cleanliness`, `revenue_anomaly_adr`, `revenue_anomaly_adr_per_room`, `revenue_apt_zero`, `data_contract_breach`, `dbt_test_failure`, `finance_flow_stale`.
