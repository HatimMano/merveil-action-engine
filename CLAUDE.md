...
⛔ **2026-09-09 — le push envoie un `min` SEUL** (ADR) : `_load_targets` traite `max_price = NULL` comme le cas **nominal** (plus comme une anomalie à recaler en -1), l'état voulu garde `max: None`, le garde-fou bornes ne borne alors que le min (`PRICE_FLOOR ≤ min ≤ PRICE_CEILING`), le diff compare `None` à `None` (`_max_of`, l'ancien `or 0` confondait « pas de plafond » et « plafond à 0 »), et le payload part avec `max-price: null` — format déjà utilisé par 217 des 293 règles vivantes chez Beyond. ⚠ `BEYOND_PRICE_FLOOR/CEILING` est un filet **technique**, indépendant du plafond métier retiré : il reste. ⚠⚠ **Ce service se déploie AVANT `merveil-dbt`** — il est rétro-compatible (max non-NULL = comportement d'avant), l'inverse retire 75 fenêtres de protection de Beyond.
...
- **Étape 3b — dérive calendrier** (`_load_calendar_drift`, `src/handlers/beyond_push.py`) : la liste de règles renvoyée par le GET n'est pas la vérité, c'est le `min_price_user` du calendrier (`beyond_raw.raw_beyond_prices`, snapshot 7h) qui protège la nuit. ⛔ **Le « 2-4 % de pushes jamais appliqués » du matin était un artefact, réfuté l'après-midi** : ABO52-2D 29/09 + 07/10 poussés le 11/09 20h46, résa 61995 (30/09→07/10) annulée à 21h25, **retirés par notre propre run de 06h46** (`remove ok` = la règle était encore dans le GET) — la fenêtre a vécu 10 h entre deux snapshots de 7 h, jamais observable ; ALF6-4F 10/09 = même schéma (add 09/09 11h04, remove 10/09 06h47, snapshot après). Contrôle élargi : toutes les fenêtres vivantes depuis le 20/08 observées par un snapshot postérieur au push → **0 nuit sous plancher**. ⚠ **L'onglet History de Beyond attribue TOUTES les règles à « Developers API »** après chacun de nos PATCH (le endpoint remplace la liste entière, règles équipe comprises) — Arnaud y a lu que nous écrivions la règle `190 € 01/09→31/10 repeat yearly` ; elle est dans le calendrier depuis le 13/05 au plus tard (1er snapshot), 3 mois avant notre 1er push, et on n'écrit jamais `rollover: true`. Une règle Repeat Yearly ou > 2 nuits n'est jamais de nous. Et la règle Yearly **ne prime pas** sur nos règles datées : 24/09 et 16/10 publient 441 dans sa plage. Le détecteur ne lit que le **cycle de vie courant** d'une fenêtre (pushes postérieurs au dernier `remove`). Pour chaque fenêtre voulue poussée **avant le jour du snapshot**, si une nuit vendable publie un min < plancher − 0,5 → la clé entre dans `to_update` (re-PATCH forcé, log `context = 'drift calendrier <nuit>: min_user=X < plancher Y'`) ; si le log porte déjà un `drift…` sur cette fenêtre et qu'elle dérive encore → **erreur mail** « plancher NON APPLIQUÉ par Beyond malgré un re-PATCH — à voir avec Arnaud ». Une fenêtre absente de Beyond suit le chemin `to_add` existant. Heartbeat `check` : `… · N dérive(s) calendrier`. Au 14/09 la requête rend 0 ligne sur les cibles du jour.
- ⚠ Piège BigQuery rencontré : une sous-requête `(SELECT d FROM snap)` dans un prédicat de `JOIN … ON` est refusée (« Unsupported subquery with table in join predicate ») → `CROSS JOIN snap` + condition en `WHERE`.
- ⚠ Ne pas confondre avec `price_posted` (= `price` du snapshot précédent, un jour de retard structurel) : la dérive se mesure sur `min_price_user`, pas sur le prix publié.
...
⭐⭐ **Porte en `on` + 3 critères + Livrer — chantier D, 2026-09-07 (ADR, décision Hatim).** `ISEO_HOLD_MODE=on` par défaut dans `deploy.sh` (observe = `ISEO_HOLD_MODE=observe ./deploy.sh`). `_evaluate_hold` a **7 critères** : les 3 historiques (direct ≤ 72 h · direct solde dû · paiement refusé sans encaissement) + **combo fraude** (`int_reservations__risk.is_fraud_combo`), **blacklist confirmée** (`int_reservations__blacklist_match`, exact/fort + actionnable), **réservé le jour J** (tous canaux, `DATE(created_at, Paris) = checkin_date`), **groupe jeune** (`f_groupe_jeune`, âges Duve). ⭐ Sur une clé M, la porte est **ré-évaluée à l'arrivée du formulaire** dans `_resa_duve_retry` (phase `form_arrival`, `_mark_held`) : les signaux nés du pré-checkin (groupe jeune, nom de pièce) retiennent le push exactement comme au provisioning. ⭐ **Libération automatique** : les retenues sont ré-évaluées à chaque run dans `_resa_duve_retry` — plus aucun critère → push seul (`released_by = auto:criteres_leves`, journal `auto_release`) ; sinon attente de [Livrer] — lus dans `_STAYS_CTE` (`risk_flags`, `blacklist_flags`), miroir `dash_ops_hold_simulation`. Une rétention = code créé, **pas poussé** ; la RC lit 6.1 (chip 🔒 Hold, motif sous la cellule) et clique **[Livrer le code]** (`POST /api/iseo/release-hold` côté dashboards : push Duve immédiat si le service porte `DUVE_CONNECT_TOKEN`, sinon `released_at` et `_resa_duve_retry` pousse au run suivant ≤ 10 min) ou **[Révoquer]**. ⚠ Le flip est sûr parce que E garantit que le code existe et se lit en 6.1.

⭐⭐ **Créer à J-3 quoi qu'il arrive — chantier E, 2026-09-07 (ADR, décision Hatim, sans shadow).** Le provisioning n'exige plus la résa Duve. Sans pré-checkin, la ligne de cache est keyée **`M<n° résa Mews>`** (n° de la 1re résa du stay), extIds `MERVEIL_RESA/USER/INV - M<n°>` — mêmes helpers, zéro migration (les lignes Duve gardent leur clé, à vie). `_provision` ne pousse rien (pas une erreur) ; **`_resa_duve_retry` joint `stays`** (par `canonical_duve` OU clé M) et pousse dès que `member_duve_ids` existe → auto-push à l'arrivée du formulaire (+ purge du DUVE_PIN natif). `duve_stay` = **staging ∪ raw 3 j** (le staging est incrémental ~2 h — sans le raw, le formulaire n'était vu qu'à la cascade). Resync et archive joignent aussi par clé M. **Garde quota** `ISEO_QUOTA_MAX_USED_NO_FORM=590` (lecture `stg_iseo__wallet`, +2 par création dans le run) : au-dessus, pas de création pour les sans-formulaire — statu quo. `_notify_hold` (observe) dit « créé, non transmis » sur une clé M. État dbt `awaiting_form` → 6.1 `attente_form` (« dicter si le client arrive », Problème à J0). ⚠ Un id Duve est hexadécimal : `LIKE 'M%'` ne peut pas le confondre. ⚠ F (54 invitations/séjour) reste à décider — chaque création sans form coûte 2 éléments.

⭐ **Vérification post-écriture — chantier C, 2026-09-07 (ADR), SANS mail.** Phase « verify » en tête de `_run_inner` : lignes de cache actives sans `lock_written_at`, créées depuis ≥ 3 min → `GET /api/v2/standardDevices/{id}` → `credentialRule.state`. `UPDATED` → `lock_written_at` ; sinon à ≥ 10 min → `_retry_push` sur la passerelle de la serrure du cache (`stg_iseo__smart_locks.gateway_id` via `iseo_lock_id`) + `write_retries += 1`, plafond `ISEO_WRITE_RETRY_MAX=3` → dbt rend **`non_ecrit`** (6.1 « Problème », dicter le code fixe). On continue de vérifier après le plafond : l'état redevient `delivre` seul quand la passerelle repart. **Scheduler `merveil-action-engine-iseo-10min` (`*/10`)**, le `:45` reste le filet ; ⚠ l'étape 5 (relance des passerelles `push_stuck`) n'est exécutée qu'aux minutes ≥ :40, sinon 12 pushes/passerelle entre deux snapshots ETL. ⛔ Aucun mail (décision Hatim) : la RC lit 6.1 chaque matin, le geste est le même quelle que soit la cause ; le trigger IT `iseo_code_non_propage` reste le canal « cause ». Colonnes cache `lock_written_at TIMESTAMP` / `write_retries INT64` (DDL 07/09). ⚠ **`GET /api/v2/standardDevices/{id}` répond 400** chez Sofia et **`/extId/{ext}` renvoie un DTO SANS `credentialRule.state`** (recette 07/09 : 98/98 en 400, puis 98/98 « rule_state=? » relancés à tort) → `_device_states()` lit **le listing paginé** (`?page=&pageSize=200`, `content`/`totalPages`, comme l'ETL) une fois par run et confronte par `id` ; relance **par passerelle** (pas par résa), UPDATE cache **par lot** (un UPDATE BQ ≈ 2-3 s, 98 par ligne = task-timeout dépassé). ⚠ Le mail « N erreur(s) » ne part qu'aux minutes ≥ :40 ; toutes les erreurs sont au log.

⛔ **Révocation humaine d'un code — 2026-09-06 (ADR).** Le dashboard (`POST /api/iseo/revoke-code`, 6.1 + Vision 360, internes seulement) supprime le code de séjour chez Sofia avec la **même séquence que `_archive`** (device par extId + invitation + user dédié) et marque la ligne de cache `archived_at` **+ `revoked_at` / `revoked_by` / `revoke_reason`** (3 colonnes ajoutées à `merveil_pin_cache` le 06/09). Cas fondateur : Muguerza Bengoechea (OPE18-4D) — la RC retire le code fixe et blackliste, le code DWH reste valide jusqu'au check-out. ⚠ **L'orchestrateur ne recrée JAMAIS un code révoqué** : `_resa_to_provision` (CTE `active`) et `_whitelisted_gaps` (`active_by_mews`) traitent une ligne `revoked_at IS NOT NULL AND checkout_date >= today` comme active — sans ça le run suivant remettrait un code à la personne qu'on vient de couper, et le mail quotidien l'accuserait d'être « manquant ». ⚠ Rien d'automatique sur une fiche blacklist `interdiction` (un client en séjour signalé pour comportement reste jusqu'à son départ) ; l'annulation Mews reste l'autre chemin, via `_resa_to_archive` au run suivant. Journal : `iseo_raw.code_revoke_log`.
...
⭐⭐ **LA LOGIQUE PAIEMENT A QUITTÉ CE FICHIER — 2026-09-09 (ADR).** `_PAYMENTS_CTE` n'est plus qu'une lecture de la vue dbt **`int_reservations__payments`** (env `PAYMENTS_TABLE`), SEULE SOURCE « dû / encaissé / solde » du DWH — la porte, 6.1 (`payment_unpaid`), 6.7 (`f_solde_du`), le 360 (`encaisse_ttc`) et `dash_ops_hold_simulation` la lisent toutes. Elle avait fini en **5 copies**, dont celle de `dash_ops_arrivals` déjà divergente (elle ne comptait que par `reservation_id`) : la porte pouvait retenir pour non-paiement un séjour que 6.1 affichait réglé. ⚠ **Ne PAS recopier une CTE paiement dans un modèle ou un handler** — un besoin nouveau = une colonne de plus dans la vue.
- **3ᵉ voie de rattachement : le `bill`.** Quand la carte du client est refusée, la RC re-passe l'encaissement sur le compte de facturation OTA : le paiement n'a alors ni `reservation_id` ni compte `Customer` (cas Ariel Taylor 59579 — refusée à 06:57:02 et 06:57:50, encaissée **48 s plus tard** sur un compte `Company`) et les deux chemins historiques le manquent tous les deux. Il porte en revanche le `bill_id` des order items, mono-résa dans 33 429 cas sur 33 528.
- ⛔⛔ **LA VOIE BILL EST UN REPLI, PAS UNE ADDITION.** Mews enregistre le MÊME argent deux fois sur une résa OTA : l'encaissement carte (`CreditCardPayment`) et la trace du versement OTA (`ExternalPayment`) — deux `payment_id` distincts, même montant, parfois sur le même bill à 29 min d'écart (blaine squires 59770), parfois sur deux bills à trois semaines d'écart (Christy Danna 27158, 3 299,17 → 6 598,34 = ×2 pile). Aucune dédup par `payment_id` ne peut les rapprocher. Un `amount_charged` gonflé rend `balance_due` négatif et ferait passer une résa impayée pour réglée : **c'est l'erreur qui OUVRE la porte**. D'où : bill appliqué uniquement si les voies 1-2 ne voient rien, + dédup par montant en centimes. **Sous-compter est sûr** (la porte retient, un humain tranche), sur-compter ne l'est pas.
- ⚠ La vue est une **vue sur des vues de staging** : la porte voit un encaissement dès que l'ETL l'a écrit, sans attendre la cascade dbt de `:15`. Ne pas la matérialiser en table.
- Mesure (critère exact de la porte, CI −120 j → +30 j, 3 997 résas) : **15 → 11** rétentions, 4 levées, 7 encaissements correctement rattachés, 0 perdu, **0 nouveau créditeur**.

⛔⛔ **`_resa_to_resync` PERDAIT LE CRITÈRE PAIEMENT — corrigé 2026-09-09.** Son `SELECT` ne portait pas `s.payment_unpaid` : il n'avait pas suivi la refonte du 25/08 qui a fait du paiement un critère de la porte. `_evaluate_hold` lisait donc `row.get("payment_unpaid")` = `None` **à chaque resync**, et le critère disparaissait sans laisser de trace. Mesuré le 09/09 : **Anne Elizabeth Harris (52172), retenue pour 4 500 € impayés, libérée à 10h01 par le resync déclenché par son achat d'un early check-in à 255 €.** Acheter un service suffisait à contourner la porte — exactement la faille déjà documentée pour les dates (réserver à J+5, avancer à J-0), sur un autre critère. ⚠ **Leçon générale** : tout critère ajouté à `_evaluate_hold` doit être ajouté aux **quatre** requêtes qui alimentent ses `row` (`_resa_to_provision`, `_resa_to_resync`, `_resa_duve_retry`, et le rendu du mail) — un signal absent du dict vaut `False`, silencieusement.

⛔⛔ **UNE FOIS LE CODE LIVRÉ, LA PORTE NE PEUT PLUS RIEN RETENIR — 2026-09-09.** La porte retient le **push Duve**, pas le code. Or au resync le PUT conserve le **même PIN** et le **même code d'invitation** : ce qu'on pousserait est identique au caractère près à ce que le champ Duve porte déjà, sur une fenêtre que l'étape 2 vient d'élargir côté Sofia. Retenir revient à ne pas réécrire une valeur inchangée. ⚠ **Ne PAS justifier ça par « le message Duve est figé »** (correction Hatim 09/09) : à terme le message ne portera que le lien vers la Guest App, qui lit le champ **en direct** — c'est l'égalité des valeurs qui rend la rétention inopérante, pas l'immuabilité du message. ⚠ Seule exception, et elle plaide pour **pousser** : si le PUT échoue et que `_resync` retombe sur DELETE + re-POST, l'invitation change de code donc le lien change — ne pas pousser laisserait un lien mort dans Duve. Retenir là ne protégeait rien : ça remettait `duve_pushed_at` à NULL (`_save_resynced(duve_ok=False)`) — donc **6.1 affichait « retenu » sur un client qui a son code et l'a peut-être déjà utilisé** — et envoyait un mail « Livrer le code » pour un code déjà livré. Le geste qui agit sur un code sorti est **Révoquer** (point 10), pas la porte. Désormais, si `cache_pushed` : la décision est **journalisée** (`outcome = "skipped: code déjà livré"` — le signal fraude/créance reste mesurable) et la rétention est abandonnée ; **séjour déjà commencé → aucun mail** (« vérifier avant l'arrivée » n'a plus d'objet, et rien de la porte ne fera sortir le client) ; **séjour à venir → mail dans sa variante « code déjà envoyé »** (`_notify_hold(..., retenu=False)`), qui porte le bon geste — vérifier l'identité, faire annuler, changer le code — et non « Livrer ». ⚠ Trouvé en ajoutant `payment_unpaid` au resync : sans ce garde-fou, **Matilda Reaburn** (37118, SEN18-2G, Expedia VCC impayée, **en séjour** jusqu'au 10/09) recevait un « code retenu » alors qu'elle était dans l'appartement.

⭐ **LA BOUCLE DE RETRY — réécrite le 2026-09-09 (ADR), deux défauts.**
- **Une libération manuelle sans pré-checkin était EFFACÉE à l'arrivée du formulaire.** Sur une clé `M<n°>` libérée par [Livrer], la 1ʳᵉ branche (`cache_hold and not cache_released`) était sautée, la branche `elif key.startswith("M")` réévaluait la porte **sans regarder `cache_released`**, re-posait le hold via `_mark_held` — qui **n'effaçait pas `released_at`**. Résultat : le code ne partait jamais, l'overlay 6.1 affichait `attente_form` (car `released_at` posé) sur une résa retenue, le bouton disparaissait du 360, et `_notify_hold` était appelé **sans** `_hold_already_notified` → **un mail à `hello@` toutes les 10 minutes jusqu'au check-out**. Désormais : seul un motif **nouveau** (clé absente de `_motif_keys(cache_hold)`) retient, et il repart d'une rétention propre via `_mark_held(..., reset_release=True)`.
- **L'auto-libération était évaluée APRÈS le test `members`** : une clé `M` sans formulaire faisait `continue` avant, donc restait affichée « retenu · paiement refusé » alors que le paiement était encaissé. Elle est maintenant évaluée en premier — la ligne passe `released_by = auto:criteres_leves` même sans rien à pousser.
- ⚠ **Motifs comparés par CLÉ, jamais par libellé** : `_HOLD_LABELS` / `_hold_motifs` / `_motif_keys` séparent la clé (`payment_unpaid`) du texte affiché, qui porte un détail variable (`(2570 € dus)`). Sans ça, le même motif à 2 400 € paraîtrait nouveau. Pas de colonne `hold_keys` en base : la clé se retrouve par préfixe de libellé, donc **aucun DDL**. Un libellé inconnu (motif retiré du code) ne matche rien → vu comme nouveau → retient : le sens prudent.
...
- ⭐ **Toute rétention envoie un mail, mode `observe` COMPRIS** (`_notify_hold`, corrigé le 15/08 — il n'était appelé qu'en mode `on`, donc en `observe` la décision était journalisée sans que personne ne l'apprenne). Le mail dit explicitement, dans chaque mode, **si le client a le code ou non** : ce sont deux gestes RC opposés (`🔒 Code retenu à valider` vs `⚠️ Résa à risque (code envoyé)`). Règle générale : **hold ⇒ alerte**, sans quoi la porte transforme une fraude évitée en client dehors à 22 h. ⚠⚠ **Le bouton de ce mail pointait sur `?tab=arrivees` — un id d'onglet qui n'existe pas** (corrigé 09/09) : 6.1 s'appelle **`arrivals`**, et ops-front retombe **silencieusement** sur le premier onglet autorisé (6.0 Disponibilités) quand le `tab` reçu ne matche rien — le destinataire atterrissait donc ailleurs, sans erreur ni indice. Le lien passe désormais par **`_lien_arrivees(customer_name)`** → `?tab=arrivals&day=every&q=<nom>` : `q` filtre 6.1 sur le nom du client (et son email), et **`day=every` est indispensable** — 6.1 n'affiche par défaut que les arrivées J→J+4, donc un séjour déjà commencé ou plus lointain resterait invisible malgré `q`. ⚠ **Ces URLs codées en dur échappent au test `assert_dashboard_links_valid`**, qui ne contrôle que les `action_url` des triggers dbt — vérifier à la main contre le seed `dashboard_routes` (le même défaut dormait dans `webhook-gateway/src/core/risk_alert.py`). ⭐⭐ **Fusion avec `[Merveil Fraude]` (09/09, décision Hatim — cas Adam Jerou 61757 : « 🔒 Code retenu » à 12h21 puis « [Merveil Fraude] » à 12h50 pour la même résa).** Les deux mails lisent **le même verdict** `int_reservations__risk.is_fraud_combo` (motif `fraud_combo` de la porte depuis le chantier D · trigger dbt `fraude_identite`) ; ce qui différait, c'était le canal — la porte poste à l'instant de la décision, le trigger passe par un bucket bufferisé 2 h — et la diffusion (rétention = `hello@` + Hatim ; fraude = Emilia, Philippe, Hatim, `hello@`, `externe-onepilot@`). Désormais **quand la porte retient pour `fraud_combo`, son mail est LE mail fraude** : il prend les destinataires du bucket `fraude` (`_fraude_recipients()`, lus dans `routing.yaml`, jamais recopiés) et le détail des signaux (`_fraud_reasons()` → `risk_reasons` de 6.7, ligne « Signaux fraude »), et le trigger `fraude_identite` se tait par anti-join sur `hold_decisions` (`outcome IN ('held','pushed_observe')` + `hold_reason LIKE '%combo fraude%'`). ⛔ **L'anti-join ne porte QUE sur les outcomes qui ont réellement envoyé un mail** : un `skipped: whitelist` (cas Carole Holmes / Douaa Elhaddek le 09/09) n'envoie rien, et sur ces ~51 apparts non intégrés le trigger fraude est le seul filet. ⚠ Résiduel assumé : combo apparu entre le flush de `:50` et la porte de `:45` suivante → le mail fraude part d'abord et le mail de rétention suit (il porte Livrer/Révoquer, on ne le coupe pas) ; un combo qui grossit après le mail de rétention ne re-sonne plus. ⛔ Élargir la diffusion de ce mail n'est possible que parce qu'il **ne porte aucun code de porte** — condition du bucket fraude, à préserver si on y ajoute une colonne. ⭐ **Destinataires alignés le 09/09 (décision Hatim)** : `ISEO_HOLD_ALERT_TO` = bucket `fraude` = **`hello@` + Hatim + `externe-onepilot@`** — OP reçoit TOUTES les rétentions (pas seulement le combo), Emilia lit `hello@`, Philippe sorti du bucket `fraude` (il reste sur `chargeback`). ⚠ **Pourquoi la règle reste « la porte a réellement envoyé un mail » et non « l'appart est intégré »** : la porte ne réévalue une résa que sur rétention active, arrivée du formulaire ou changement de dates — un combo apparu APRÈS le provisioning sur un code déjà livré n'est jamais revu, et seul le trigger fraude le rattrape (idem skip serrure / erreur Sofia). Un découpage par périmètre rendrait ces cas silencieux sur les 75 apparts intégrés. Doublon résiduel : combo basculant dans les ~5 min entre `:45` et `:50` (~4 %), mail fraude puis mail de rétention. ⭐ **Blacklist (09/09, décision Hatim) : PAS de fusion, mais OP ajouté au bucket `blacklist`** — `trigger_blacklist_resa` sonne sur l'ÉVÉNEMENT (résa posée / fiche posée, parfois des semaines avant), la porte retient à J-3 : deux moments, deux gestes (annuler avant · Livrer/Révoquer), et le premier est le seul mail sur les apparts non intégrés. Bucket `blacklist` = Emilia, `hello@`, Hatim, Mickael, Raphael, **`externe-onepilot@`**. ⭐⭐ **`deploy.sh` déploie par DIGEST depuis le 09/09** : après le build, `IMAGE` est réécrit en `…@sha256:…` avant `COMMON_ARGS` — `jobs deploy --image …:latest` ne créait aucune révision quand la spec portait déjà cette chaîne (2ᵉ build du jour = rien déployé, sans erreur ; vu 3× le 09/09). Plus besoin de re-pointer les 7 jobs à la main. ✅ `ISEO_HOLD_ALERT_TO` = **`hello@merveil.co,hatim@archides.fr` depuis le 02/09** (volume mesuré ~2-4 mails/mois sur `hold_decisions` ; le mail ne porte AUCUN code de porte — compatible boîte lue par des externes).
