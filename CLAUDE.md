...
**Label Luckey — guest tag dédié « Merveil Guest » (2026-06-29)** : `_provision`/`_resync` forcent `ISEO_DEFAULT_GUEST_TAG_ID` (**184106**) au lieu du guest tag dominant de l'appart. Cause : chaque guest tag Sofia porte UN user ; le tag dominant d'un appart appartient à un vrai guest passé → tous nos PIN héritaient de son nom dans l'UI Luckey (ex: PIN de Muller/Henderson affichés « Bianca Aranha » sur SEB23-3G). Le credentialRule scope l'accès par le lockTag → le guest tag n'a aucun effet de bord. Tag 184106 « Merveil Guest » + user 182843 (champ tags Sofia, pas `userTags`) créés via `merveil-etl-v2/utils/iseo_setup_merveil_guest.py` (endpoints `POST /api/v2/userTags` + `POST /api/v2/users` roleIds=[5], confirmés dans `docs/apis/ISEO_api.pdf` + OpenAPI live). Les PIN actifs déjà créés gardent leur ancien tag jusqu'au checkout (pas de re-provision = pas de changement de code). Le vrai nom guest reste sur le dashboard (dérivé Duve). NB ancien fallback 132094 = compte manager `sylvain@archides.fr`, abandonné (non renommable).
