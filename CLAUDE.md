...
| iseo_gateway_push_stuck | email_digest@daily | ✅ | 10 au 25/08 dont 5 CRITICAL — passerelle qui RÉPOND au cloud mais dont le dernier push `CREDENTIALS_UPDATED` n'a pas abouti. CRITICAL si un appart **intégré** est derrière (le provisioning y est bloqué, les clients entrent au code fixe), WARNING sinon. ⚠ **Complémentaire de `iseo_gateway_offline`, pas redondant** : celui-ci lit la connexion, donc ne voit que les débranchées — les 6 passerelles de l'incident du 08-24/08 pinguaient toutes normalement. Bucket daily : état persistant, cf. la leçon des 8 mails nocturnes du 20/08 |
